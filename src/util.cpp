                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No datacoin.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override datacoin.conf
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "Datacoin:" << FormatVersion(DATACOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}

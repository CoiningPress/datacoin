...
const int DATACOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString DATACOIN_IPC_PREFIX("datacoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(DATACOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(DATACOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(DATACOIN_IPC_CONNECT_TIMEOUT);
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click datacoin: links)
...
{
    // clicking on datacoin: URLs creates FileOpen events on the Mac:
